...
Index Terms - Multi-Agent Reinforcement Learning (MARL), Emissions reduction, Proximal Policy Optimization (PPO),
Deep Q-Network (DQN).
\end{abstract}
...
PPO as an algorithm is a reasonable choice as it's very sample efficient when learning which
is important because getting training examples is a very time consuming project.
Even though we didnt manage to make use of real world examples to train, this might be relevant
in the future if this project is expanded upon. It is shown later in this paper that DQN learns
faster than PPO in our specific project, which is to be expected.


\subsection{Multi-agent DQN}
We attempted a version of DQN where the reward for an action is given as the average reward for
...
maximise their own reward is significantly more effecient.
